...
const Homes = async() => {
  ...
  return (
    <div className="w-full h-screen flex justify-center items-center">
       <div className="flex flex-col min-w-[300px] min-h-[350px] md:min-w-[400px] md:flex-row   md:min-h-[200px]  justify-center gap-5 items-center border rounded-md p-2">
          <div className="h-[300px] w-[300px] md:w-[300px] md:h-[200px]  flex justify-center text-center md:text-left flex-col rounded-md">
            <div className={`${data?.data[0]?.color && `text-[${data?.data[0]?.color}]`
             } ${data?.data[0]?.fontbold && `font-${data?.data[0]?.fontbold}`} ${data?.data[0]?.fontsize && `text-[${data?.data[0]?.fontsize}]`}`}>{data?.data[0]?.title}</div>
            <div className={`${data?.data[0]?.desccolor && `text-[${data?.data[0]?.desccolor}]`
             } ${data?.data[0]?.descfontbold && `font-${data?.data[0]?.descfontbold}`} ${data?.data[0]?.descfontsize && `text-[${data?.data[0]?.descfontsize}]`}`}>{data?.data[0]?.desc}</div>
          </div>
